...
const handleFavorite = async (songId, user) => {
  ...
  try {
    if (user?.favorites?.length === 0) {
      ...
      if (songId && user?._id) {
        const response = await fetch(`http://localhost:5001/api/addtofav/${user?._id}`, {
          method: 'PUT',
          headers: {
            'Content-Type': 'application/json',
          },
...
const removeFavorite = async (songId, user) => {
  ...
  try {
    if (user?.favorites[0]?.songs.length === 0) {
      ...
      if (songId && user?._id) {
        const response = await fetch(`http://localhost:5001/api/removefromfav/${user?._id}`, {
          method: 'PUT',
          headers: {
            'Content-Type': 'application/json',
          },
...
const handlePlaylist = async (songId, selectedPlaylists ,user) => {
  ...
  try {
    if (user?.favorites?.length === 0) {
      ...
      if (songId && user?._id) {
        const response = await fetch(`http://localhost:5001/api/addtoplaylist/${user?._id}`, {
          method: 'PUT',
          headers: {
            'Content-Type': 'application/json',
          },
